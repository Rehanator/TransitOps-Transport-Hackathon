  ...
  return (
    <div className="space-y-6">
      <header className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <p className="text-xs uppercase tracking-widest text-muted-foreground">Consumption</p>
          <h1 className="text-3xl font-bold tracking-tight">Fuel Logs</h1>
        </div>
        <div className="flex gap-2">
          <Button variant="outline" onClick={() => exportCSV("fuel.csv", rows)}>
            <Download className="mr-2 h-4 w-4" /> CSV
          </Button>
          {canEdit && (<Button onClick={() => setOpen(true)}><Plus className="mr-2 h-4 w-4" /> New Log</Button>)}
        </div>
      </header>
